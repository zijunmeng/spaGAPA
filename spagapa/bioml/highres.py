...
def highres_bioml_recover(
    ...
    if domain_method == "spectral" and fused_graph.shape[0] > 5000:
        notes.append(f"domain_method auto-switched spectral to leiden (n_spots={fused_graph.shape[0]} > 5000)")
        domain_method = "leiden"
    ...
    elif domain_method in ("leiden", "kmeans"):
        use_leiden = domain_method == "leiden" or fused_graph.shape[0] > 5000
        if use_leiden:
            try:
                import igraph as ig
                import leidenalg
                coo = fused_graph.tocoo()
                g = ig.Graph(n=fused_graph.shape[0],
                ...
                notes.append(f"Leiden on fused_graph: {len(np.unique(labels))} domains")
            except ImportError:
                notes.append("igraph/leidenalg not available - falling back to kmeans")
    ...
    domain_counts = np.bincount(labels)
    max_fraction = domain_counts.max() / len(labels)
    if max_fraction > 0.8 and len(domain_counts) > 3:
        notes.append(
            f"WARNING: domain collapse - largest domain has {max_fraction:.1%} of spots. "
            "Consider domain_method='leiden' or providing an expression matrix."
        )
