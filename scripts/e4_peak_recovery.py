...
def main():
    ...
    saf_k = saf[saf["peakID"].isin(set(keep))].copy().reset_index(drop=True)
    ...
    m = np.isin(pk, np.fromiter(keep_idx, dtype=np.int64))
    pk, bk, vv = pk[m], bk[m], vv[m]
    pk = np.array([pidx_k[saf.iloc[i]["peakID"]] for i in pk], dtype=np.int64)
    ...
    with open(OUT / "apa_matrix.csv", "w") as fh:
        ...
        for i, pid in enumerate(prow):
            fh.write(pid + "," + ",".join(f"{v:g}" if v else "0" for v in arr[i]) + "\n")
    del arr
