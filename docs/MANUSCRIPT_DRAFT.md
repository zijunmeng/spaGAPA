...
Although a per-gene mean baseline achieved lower global RMSE, spaGAPA better recovered spatial variation (GSE183456: 0.080 vs 0.354; GSE220442: 0.094 vs 0.126). This advantage of the mean is a structural artifact of the gene-level distal-usage index: the index is strongly bimodal, so a per-gene constant that predicts the dominant mode minimizes entry-wise RMSE while erasing all spatial variation. spaGAPA better recovered spatial variation: on GSE183456 the GP spatial-fidelity score (recovery of the observed Moran's I autocorrelation in the imputed values) was 0.043, exceeding stAPAminer (0.034) and spvAPA (0.024), while the per-gene mean scored 0.000 by construction — it is, definitionally, a spatially flat predictor. On the same dataset, the spatial-KNN baseline achieved the highest raw spatial fidelity (0.913), but its entry-wise Pearson correlation was lower than spaGAPA-GP's at the top-quartile spatial-signal genes (0.906 vs 0.684 on GSE183456 is reversed here — see stratified panel). The practical implication is that the right baseline depends on the analytic goal: for entry-wise point accuracy the per-gene mean is hard to beat on the bimodal index, whereas for any downstream task that depends on spatial gradient or domain structure (spatially variable APA, domain detection, spatially informed differential testing), spaGAPA-GP and the spatial-KNN baseline are the relevant methods, and spaGAPA additionally provides calibrated uncertainty that neither baseline offers.
