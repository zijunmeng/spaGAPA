...
**19 samples drawn from 10 independent GSE studies (12 tissue contexts — 4 retinal replicates counted as one), 12.7M+ test points, 7 GSE (Visium) + 3 GSE (Stereo-seq), 3 species (human/mouse/rat), 2 callers (scAPAtrap/Sierra):**
...
2. **Raw GP uncertainty correlation is modest within genes** (per-gene median r ≈ 0.10–0.19; pooled r ≈ 0.5 partly driven by cross-gene heterogeneity). Residual-based heteroscedastic estimation (method D) clears pooled r ≥ 0.3 and enables risk-coverage triage; conformal calibration provides distribution-free marginal coverage regardless. Simulation benchmarks confirm GP posterior std carries ranking information (r = 0.37–0.44 in controlled settings), suggesting the lower real-data correlation partly reflects biological noise rather than method failure.
3. **Marginal ≠ conditional coverage** — high-expression bins undercover (~0.82); the guarantee is marginal, not per-subgroup (shown explicitly in Fig 4E / Supp Fig S9). Mondrian (group-conditional) conformal calibration, implemented in the v2 branch, mitigates high-expression undercoverage; integration into the main figure set is pending.
4. **Stereo-seq raw FASTQ + mask availability is a field-wide bottleneck, but improving** — STOmics chip masks are retained on OSS only for a limited window after each experiment and expire for published datasets (documented with official reply: [STOmics/SAW#268](https://github.com/STOmics/SAW/issues/268); GSE269906 human AD brain is mask-blocked for this reason). Mitigation demonstrated (2026-09): a systematic GEO survey (75 series → 28 with DNBSEQ raw data) identified datasets whose authors deposited `barcodeToPos.h5` directly in GEO supplementary; via that route spaGAPA now runs on human retinal-organoid (GSE293464, 4 samples) and rat thymus (GSE333693) Stereo-seq data with conformal coverage within ±0.07 pp on all 30 sample×level×mode combinations (11.3M new test points).
5. **Cross-sample conformal transfer shows tissue-dependent variance** — leave-one-sample-out transfer (110 pairs, 11 samples) yields mean decay <0.4 pp at all levels, but individual cross-tissue pairs deviate up to 11.8 pp at 80% nominal. The 95% level is most robust (max single-pair deviation 6.75 pp). Calibration is sample-dependent for distant tissue pairs, though transferable within similar tissue types.
6. **Simulation reveals non-monotone depth scaling** — at fixed capture fraction, increasing spots from 5k to 80k does not monotonically improve accuracy: at 80k with 500 inducing points, capacity limits slightly degrade RMSE and spatial fidelity. Higher inducing budgets are needed for very large tissues at high observation rates.
7. **Genome version mismatch is a practical pitfall for cross-resource PAS comparison** — two Visium mouse datasets aligned to GRCm39 showed only ~6% overlap with the mm10-based PolyA_DB v4.1 catalog before UCSC liftOver correction (→77–87% after, <0.11% unliftable). Cross-resource APA validation requires explicit coordinate system alignment.
8. **sAPA-RegNet annotation receives partial natural-perturbation support** — the CFIm25/NUDT21 positive control is direction-concordant across spatial domains (shuffle p=0.003, FDR=0.020, consistent with Masamha et al. 2014 *Nature*), and NOVA1/KHSRP also pass FDR<0.10 (12 RBPs tested, 4-sample retinal-organoid data). However, PTBP1 — the strongest literature prior — is contradicted in retinal organoids (site-gene consistency below no-site background), and cross-RBP motif specificity is weak (off-diagonal ≈ diagonal). A pre-registered holdout test (200 random splits, 8 train / 4 held-out RBPs) confirms this: held-out direction consistency is 0.37 — significantly BELOW chance (p=9.2e-12), with all four distal-promoting priors concordant but all seven proximal-promoting priors failing. The annotation layer is therefore hypothesis-generating with demonstrated sensitivity to known regulators (CFIm25 positive control passes) but no established direction specificity; it is presented in the supplementary materials. Spot-level perturbation regression was separately tested in a go/no-go framework and found unsupported; that model is not included.
9. **No supervised analysis** — spaGAPA is unsupervised; spvAPA offers supervised sPLS-DA.
10. **Batch-correction module still under evaluation** — Harmony comparison pending; listed for completeness.
